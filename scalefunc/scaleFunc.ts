...
import { encodeString, decodeString, encodeUint32, decodeUint32, encodeUint8Array, decodeUint8Array } from "@loopholelabs/polyglot-ts";
...
export class ScaleFunc {
  ...
  constructor(version:Version, name:string, signature:string, language:string, extensions:Extension[], fn:Buffer) {
    this.Version = version;
    this.Name = name;
  ...
  static Decode(data: Uint8Array):ScaleFunc {
    ...
    const sf = new ScaleFunc(version, name, signature, language, extensions, Buffer.from(fn));
    sf.Size = size;
    sf.Checksum = hash;
    return sf;
  }
